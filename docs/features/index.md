...
**ApplicationSet Support**: ApplicationSets can run on either the control plane or the agent, with generated Applications synced through the normal agent protocol. With destination-based agent mapping, ApplicationSets work seamlessly across the distributed architecture. ApplicationSet resources themselves are not synced between principal and agent, as the architecture operates at the Application level.
