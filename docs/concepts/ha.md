...
The admin server (`ha status/promote/demote`) listens on port 8405. When TLS is active, it inherits mTLS from the main endpoint, binds to `0.0.0.0`, and authorizes clients via `--ha-admin-auth`. In plaintext mode it binds to `127.0.0.1` and relies on `kubectl port-forward`.
