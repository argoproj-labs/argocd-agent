...
func Test_isResourceOwnedByApp(t *testing.T) {
	labelTrackingConfig := &ResourceTrackingConfig{
	...
	t.Run("Find manager by label", func(t *testing.T) {
		...
		isOwned, err := isResourceManaged(kubeclient, un, 5, labelTrackingConfig)
		assert.True(t, isOwned)
		assert.NoError(t, err)
	})
	t.Run("Find manager by annotation", func(t *testing.T) {
		annotationTrackingConfig := &ResourceTrackingConfig{
		...
		isOwned, err := isResourceManaged(kubeclient, un, 5, annotationTrackingConfig)
		assert.True(t, isOwned)
		assert.NoError(t, err)
	})
	...
	t.Run("Recursion limit reached", func(t *testing.T) {
		...
		isOwned, err := isResourceManaged(kubeclient, un, 3, labelTrackingConfig)
		assert.True(t, isOwned)
		assert.NoError(t, err)

		isOwned, err = isResourceManaged(kubeclient, un, 2, labelTrackingConfig)
		assert.False(t, isOwned)
	...
	t.Run("Label-only tracking method", func(t *testing.T) {
		labelOnlyConfig := &ResourceTrackingConfig{
			Method:   v1alpha1.TrackingMethodLabel,
			LabelKey: "app.kubernetes.io/instance",
		}
		...
		kubeclient := kube.NewDynamicFakeClient(pod1)
		un := objToUnstructured(pod1)
		isOwned, err := isResourceManaged(kubeclient, un, 5, labelOnlyConfig)
		assert.True(t, isOwned)
		assert.NoError(t, err)
		...
		isOwned, err = isResourceManaged(kubeclient2, un2, 5, labelOnlyConfig)
		assert.False(t, isOwned)
		assert.NoError(t, err)
	})

	t.Run("Annotation-only tracking method", func(t *testing.T) {
		annotationOnlyConfig := &ResourceTrackingConfig{
			Method:        v1alpha1.TrackingMethodAnnotation,
			AnnotationKey: "argocd.argoproj.io/tracking-id",
		}
		...
		kubeclient := kube.NewDynamicFakeClient(pod1)
		un := objToUnstructured(pod1)
		isOwned, err := isResourceManaged(kubeclient, un, 5, annotationOnlyConfig)
		assert.True(t, isOwned)
		assert.NoError(t, err)
		...
		kubeclient2 := kube.NewDynamicFakeClient(pod2)
		un2 := objToUnstructured(pod2)
		isOwned, err = isResourceManaged(kubeclient2, un2, 5, annotationOnlyConfig)
		assert.False(t, isOwned)
		assert.NoError(t, err)
	})

	t.Run("Annotation+Label tracking method", func(t *testing.T) {
		annotationAndLabelConfig := &ResourceTrackingConfig{
		...
		kubeclient := kube.NewDynamicFakeClient(pod1)
		un := objToUnstructured(pod1)
		isOwned, err := isResourceManaged(kubeclient, un, 5, annotationAndLabelConfig)
		assert.True(t, isOwned)
		assert.NoError(t, err)
		...
		kubeclient2 := kube.NewDynamicFakeClient(pod2)
		un2 := objToUnstructured(pod2)
		isOwned, err = isResourceManaged(kubeclient2, un2, 5, annotationAndLabelConfig)
		assert.False(t, isOwned)
		assert.NoError(t, err)
		...
		isOwned, err = isResourceManaged(kubeclient3, un3, 5, annotationAndLabelConfig)
		assert.False(t, isOwned)
		assert.NoError(t, err)
	})
}
...
func Test_processIncomingResourceRequest(t *testing.T) {
	...
	t.Run("Get unmanaged resource returns error", func(t *testing.T) {
		...
		agent := &Agent{
			...
			emitter:             event.NewEventSource("test-agent"),
			enableResourceProxy: true,
			trackingReader:      NewResourceTrackingReader(kubeClient, "argocd"),
		}
		require.NoError(t, agent.queues.Create(defaultQueueName))
...
func Test_processIncomingDeleteResourceRequest(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Use fixtures to set up tracking with label method
			_, reader := fixtures("argocd", v1alpha1.TrackingMethodLabel)
