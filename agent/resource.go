...
func (a *Agent) getManagedResource(ctx context.Context, gvr schema.GroupVersionResource, name, namespace string) (*unstructured.Unstructured, error) {
	...
	ok, err := isResourceManaged(a.kubeClient, res, ownerLookupRecursionLimit, a.trackingReader.GetConfig())
	if !ok {
		if err != nil {
...
// managed by an Argo CD application using the provided tracking configuration.
func isResourceManaged(kube *kube.KubernetesClient, res *unstructured.Unstructured, maxRecurse int, trackingConfig *ResourceTrackingConfig) (bool, error) {
	if maxRecurse < 1 {
	...
	// Use the tracking configuration to determine if the resource is managed
	if trackingConfig.IsResourceTracked(lbls, annt) {
		return true, nil
	}
	...
	for _, ref := range refs {
		...
		isOwned, err := isResourceManaged(kube, owner, maxRecurse-1, trackingConfig)
		if err != nil {
			return false, err
		}
