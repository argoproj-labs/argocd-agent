...
func Test_NewClusterRegistrationManager(t *testing.T) {
	t.Run("Create manager with self cluster registration enabled", func(t *testing.T) {
		...
		mgr := NewClusterRegistrationManager(true, testNamespace, testResourceProxyAddr, clientCertSecretName, kubeclient, iss)
...
func Test_RegisterCluster(t *testing.T) {
	...
	t.Run("Returns error when client cert secret is missing", func(t *testing.T) {
		...
		mgr := NewClusterRegistrationManager(true, testNamespace, testResourceProxyAddr, "nonexistent-secret", kubeclient, iss)
	...
	t.Run("Does not modify manually created cluster secret", func(t *testing.T) {
		...
		manualSecret := &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					...
					// Note: No LabelKeySelfRegisteredCluster label
				},
			},
			Data: map[string][]byte{
		...
		// Issuer should NOT be called since we should skip the manual secret
		iss := issuermocks.NewIssuer(t)
		// No expectations set - if called, the test will fail
