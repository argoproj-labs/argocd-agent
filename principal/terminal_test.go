...
func TestAgentToWebSocketChannelBehavior(t *testing.T) {
	t.Run("handles FromAgent channel close", func(t *testing.T) {
		...
		// Close FromAgent channel
		close(session.FromAgent)

		// agentToWebSocketChannel should detect closed channel and exit
		// (We're not calling it directly here as it's part of the Server struct)
		_ = session
	})
}

// Test webSocketToAgentChannel goroutine behavior
func TestWebSocketToAgentChannelBehavior(t *testing.T) {
	t.Run("handles WebSocket close", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			upgrader := websocket.Upgrader{}
			conn, err := upgrader.Upgrade(w, r, nil)
			if err != nil {
		...
		// webSocketToAgentChannel should detect closed WebSocket and exit
		// (We're not calling it directly here as it's part of the Server struct)
		_ = session
	})
}
