...
func (h *HAComponents) buildAdminGRPCServer() (*grpc.Server, error) {
...
func (h *HAComponents) StartHA(ctx context.Context) error {
	...
	adminServer, err := h.buildAdminGRPCServer()
	if err != nil {
	...
	adminHost := "127.0.0.1"
	if h.tlsConfigProvider != nil {
		// When authentication is enabled, listen on all interfaces for TLS-enabled
		// admin server, so that HAAdmin can be accessed from outside the cluster
		adminHost = "0.0.0.0"
	}
