...
With the bundled chart, the Argo CD **application controller** is created as a **StatefulSet** (default in argo-helm), named like `<helm-release-name>-argocd-application-controller`. It does not appear under `kubectl get deployment`. Use `kubectl get statefulset` (or `kubectl get pods -l app.kubernetes.io/name=argocd-application-controller`) to verify it. A **Deployment** is used for that component only if you enable argo-helm’s alpha option `argocd.controller.dynamicClusterDistribution`.
...
The Kubernetes namespace where the agent's resources (Deployment, Service, ConfigMap, etc.) and the bundled Argo CD subchart will be deployed. This can be overridden by the helm install --namespace flag. Preferred over the top-level `namespaceOverride` which only affects the agent chart.
