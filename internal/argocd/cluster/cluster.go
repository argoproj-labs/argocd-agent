...
func (m *Manager) UpdateClusterConnectionInfo(agentName, status string, t time.Time) {
	...
	redisOptions := m.configureRedisServerHost(agentName, cluster.Name)
...
// configureRedisServerHost configures redis server hostname for creating a redis cache client.
func (m *Manager) configureRedisServerHost(agentName, clusterName string) *redis.Options {
	redisOptions := &redis.Options{Addr: m.redisAddress}

	// if redis server host is provided then use that else use LoadBalancer address to access redis cache
	if m.redisAddress == "" {
		// // get service created for redis server
		service, err := m.kubeclient.CoreV1().Services(m.namespace).Get(m.ctx, common.DefaultRedisName, metav1.GetOptions{})
		if err != nil {
			log().Errorf("Failed to get redis host name for Cluster: '%s' mapped with Agent: '%s'. Error: %v", clusterName, agentName, err)
			return redisOptions
		}

		redisOptions = &redis.Options{Addr: fmt.Sprintf("%s:%s", service.Status.LoadBalancer.Ingress[0].Hostname, &service.Spec.Ports[0].TargetPort)}
	}
