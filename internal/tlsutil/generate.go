...
// CertPem encodes the certificate c's public cert to PEM
...
// KeyPem encodes the certificate c's private key to PEM
func (c *Certificate) KeyPem() (string, error) {
	keyPem := new(bytes.Buffer)
	var keyBytes []byte
	var err error

	if keyBytes, err = x509.MarshalPKCS8PrivateKey(c.key); err != nil {
		return "", err
	}
