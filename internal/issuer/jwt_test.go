...
func Test_ResourceProxyToken(t *testing.T) {
	...
	t.Run("Resource proxy token does not expire", func(t *testing.T) {
		i, err := NewIssuer("server", WithRSAPrivateKey(key))
		require.NoError(t, err)
		// Issue token and validate it - it should work regardless of time
		tok, err := i.IssueResourceProxyToken("test-agent")
		require.NoError(t, err)
		c, err := i.ValidateResourceProxyToken(tok)
		require.NoError(t, err)
		require.NotNil(t, c)
		// Check that ExpiresAt is nil (no expiry)
